...
#renumber rows to correspond with item number (only necessary if considering facet > 1)       
for i in range(n_items):
    extrm_correlations.index.values[i] = str(i)
    ...
    if x > 0: #if there are dependent items in column 1
        for j in range (x): #go through all dependent items in column 1, and add the item value of that column to the others
            ...
            if col2.isna().sum() > 0: #if there are NaNs in column2
                next_row = col2.notna().idxmin() #find first non-NaN location in row
                ...
                next_row = -1 #add item at the end of column
            dependent_items.iloc[int(next_row),int(val-1)] = int(i+1) #add dependent item to correct column
